      ...
      case Some(p: SocialProvider with CommonSocialProfileBuilder) =>
        p.authenticate().flatMap {
          ...
          case Right(authInfo) => for {
            ...
            _ <- userService.create(profile.loginInfo.providerID, profile.loginInfo.providerKey, profile.email.getOrElse(""))
            ...
            result.withCookies(Cookie(name, value, httpOnly = false),
              Cookie("email", profile.email.fold("Private")(_.toString),
                httpOnly = false))
          }
        }
